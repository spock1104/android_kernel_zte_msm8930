...
//zte jiangfeng add
#define ZTE_READ_CC	0x1

void zte_calculate_cc_uah(struct pm8921_bms_chip *chip, int cc, int *val);
static int zte_get_reading(void *data, u64 * val)
{
	int param = (int)data;
	int ret = 0;
	int zte_coulumb_counter	=	0;
	int cc_uah	=	0;
	...
	*val = 0;
	...
	case ZTE_READ_CC:
		mutex_lock(&the_chip->bms_output_lock);
		read_cc(the_chip, &zte_coulumb_counter);
		mutex_unlock(&the_chip->bms_output_lock);
		zte_calculate_cc_uah(the_chip, zte_coulumb_counter, &cc_uah);
		*val =	cc_uah*3600/1000;
		break;
	default:
		ret = -EINVAL;
	}
...
DEFINE_SIMPLE_ATTRIBUTE(zte_reading_fops, zte_get_reading, NULL, "%lld\n");
...
static void create_debugfs_entries(struct pm8921_bms_chip *chip)
{
	...
	debugfs_create_file("coulumb_counter", 0644, chip->dent,
			(void *)ZTE_READ_CC, &zte_reading_fops);		//zte jiangfeng add
...
void zte_calculate_cc_uah(struct pm8921_bms_chip *chip, int cc, int *val)
{
	...
	pr_debug("cc_voltage_uv = %lld microvolts\n", cc_voltage_uv);
	cc_pvh = ccmicrovolt_to_pvh(cc_voltage_uv);
	pr_debug("cc_pvh = %lld micro_volt_hour\n", cc_pvh);
	cc_uah = div_s64(cc_pvh, chip->r_sense_uohm);
	*val = cc_uah;
}
...
int zte_get_cutoff_voltate(void)
{
	return (the_chip->v_cutoff+50)*1000;
}
//zte jiangfeng add, end
