 ...
 */
	...
	||	defined(CONFIG_MACH_METIS)	|| defined(CONFIG_MACH_OCEANUS)	\
	||	defined(CONFIG_MACH_IRIS)
...
#include <linux/reboot.h>	/*ZTE  For kernel_power_off() */
...
struct pm8921_chg_chip {
	...
	struct delayed_work             power_supply_changed_work;  //ZTE
...
static char *charger_type[] = {
    "USB","carkit", "wall",  "wireless", "invalid charger"
};
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		val->intval	=	pm_chg_get_rt_status(the_chip, DCIN_VALID_IRQ);
		return 0;
		break;
	default:
		return -EINVAL;
...
extern int zte_get_cutoff_voltate(void);		//zte jiangfeng add
#if 0
static int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
#else
int get_prop_batt_capacity(struct pm8921_chg_chip *chip)
#endif
{
	int percent_soc, voltage,status,percent_soc_bms;
	static  int first_time_run=10;   //ZTE_XJB_PM_20120831
	static  int double_check=0;      //ZTE_XJB_PM_20120831
	static  int double_check2=0;      //ZTE_XJB_PM_20130831
	...
	{
		printk("slf low battery charge = %d%%,voltage=%d,double_check=%d\n", percent_soc,voltage,double_check);
		//#if defined(CONFIG_MACH_HAYES)||defined(CONFIG_MACH_ELDEN) //ZTE_XJB_PM_20120831 start
		if(double_check>=3)
		{
			//double_check=0;
	...
	if ((percent_soc <= 2)&&(voltage<3400000)&&(status==POWER_SUPPLY_STATUS_CHARGING))
	{
		printk(" low battery percent_soc = %d%%,voltage=%d,double_check2=%d \n", percent_soc,voltage,double_check2);
		//#if defined(CONFIG_MACH_HAYES)||defined(CONFIG_MACH_ELDEN) //ZTE_XJB_PM_20120831 start
               if(double_check2>=8)
	       {
			//double_check2=0;
	...
	if((percent_soc==0)&&(voltage > (zte_get_cutoff_voltate()+100000)))
	{
             percent_soc=1;
	      g_bms_flag=5;	 
	}
	...
	//ZTE 
        if (percent_soc <= 5)
	printk("ZTE  percent_soc=%d,status=%d ,flag_check=%d ,count_number=%d\n",percent_soc,status,flag_check,count_number);
	if(percent_soc==0)
	{
	if(!flag_check)
	...
	if (count_number >=0)	
		{
		if(percent_soc_bms>1)
			percent_soc=percent_soc_bms--;
		else
			percent_soc =1;
		g_bms_flag=7;
		}
...
//zte  add
	if(percent_soc == 100	&& (status==POWER_SUPPLY_STATUS_CHARGING))
	{
		printk("%s ZTE  force charging restore soc to 99,here. \n",__func__);
		percent_soc	=	99;
	}
//zte  add, end
...
void zte_set_chg_priority(enum zte_pm8921_chg_priority chg_priority);
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	else if (mA>500)
	{
		...
		zte_set_chg_priority(USB_FIRST);
	}
	else
	{
		zte_set_chg_priority(DC_FIRST);
		g_charger_type = DC_CHG_TYPE__WIRELESS;
	}
	//zte add,end
...
int pm8921_dc_ovp_set_threshold(enum pm8921_dc_ov_threshold ov)
{
	...
	return pm_chg_masked_write(the_chip, DC_OVP_CONTROL,
				DC_OV_THRESHOLD_MASK, temp);
}
EXPORT_SYMBOL(pm8921_dc_ovp_set_threshold);

#define DC_OVP_DISABLE_MASK	0x80
int pm8921_dc_ovp_disable(int disable)
{
	u8 temp = 0;
	...
	if (disable)
		temp = DC_OVP_DISABLE_MASK;

	return pm_chg_masked_write(the_chip, DC_OVP_CONTROL,
				DC_OVP_DISABLE_MASK, temp);
}
...
int pm8921_set_usb_power_supply_type(enum power_supply_type type)
{
	...
	power_supply_changed(&the_chip->batt_psy);   //ZTE
	//power_supply_changed(&the_chip->ac_psy);  //ZTE
	return 0;
}
...
static void update_heartbeat(struct work_struct *work)
{
	...
	else
	{
		...
		if(the_chip)
			printk("pm8921 update_heartbeat: charging_disabled = %d, charge_autoenable= %d,vddmax=%d\n",pm_is_chg_charge_dis(the_chip),zte_charge_autoenable,vdd_max);
...
#define CHANGED_CHECK_PERIOD	1000//1s
extern void power_supply_changed_zte(struct power_supply *psy);
static void power_supply_changed_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip = container_of(dwork,
				struct pm8921_chg_chip, power_supply_changed_work);

	power_supply_changed_zte(&chip->batt_psy);

	schedule_delayed_work(&chip->power_supply_changed_work,
			round_jiffies_relative(msecs_to_jiffies
						(CHANGED_CHECK_PERIOD)));
...
static void bms_check(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip = container_of(dwork,
				struct pm8921_chg_chip, bms_check_work);
	...
	int capacity,chg_current,status;
       ...
       if(chip==NULL)
		...
		capacity =get_prop_batt_capacity(chip);
		//present= is_battery_valid(chip);
		chg_current=get_prop_batt_current(chip,&chg_current)/1000;
              status=get_prop_batt_status(chip);
...
bool	low_chg_current	=	false;	//zte jiangfeng add
static void set_appropriate_battery_current(struct pm8921_chg_chip *chip)
{
	...
	if((g_charger_type!=USB_CHG_TYPE__WALLCHARGER)&&(is_dc_chg_plugged_in(the_chip)))
		{
			chg_current = min(chg_current, (unsigned int)500);//wireless charger setting 750mA
			g_charger_type = DC_CHG_TYPE__WIRELESS;
...
static void battery_warm(bool enter)
{
	...
	if (enter)
	{
		...
		is_stop_charge=1;
		printk("%s disabled charge .charge_stop_by_warm=1,is_stop_charge=1 \n",__func__);
#endif
	}
	else
	{
		...
		is_stop_charge=0;
		printk("%s enabled  charge .charge_stop_by_warm=0,is_stop_charge=0 \n",__func__);
#endif
	}
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	#ifndef ZTE_WARM_LOW_CURRENT		//no need to change voltage
	chip->warm_bat_voltage = pdata->max_voltage;
	#else
...
#if defined(CONFIG_MACH_NEX) || defined(CONFIG_MACH_STORMER)	// // P893V10, N9510
	chip->cold_thr=PM_SMBC_BATT_TEMP_COLD_THR__LOW; // 70%	//0 degreeC
	chip->hot_thr=PM_SMBC_BATT_TEMP_HOT_THR__HIGH;	// 35%	//45 degreeC
#endif
	...
	INIT_DELAYED_WORK(&chip->bms_check_work, bms_check);
	INIT_DELAYED_WORK(&chip->power_supply_changed_work, power_supply_changed_func);
	...
	pm8921_dc_ovp_disable(true);
	pm8921_dc_ovp_set_threshold(PM_DC_OV_8P5V);
