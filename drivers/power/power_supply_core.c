...
/**************************************************************************/
int is_power_supply_changed=0;
void power_supply_changed_ori(struct power_supply *psy,const char* func,int line)
{
	unsigned long flags;
	printk("[fake]psy change, func %s,line %d,psy name: %s\n",func, line,psy->name);	//ZTE jiangfeng add
	spin_lock_irqsave(&psy->changed_lock, flags);
	is_power_supply_changed = 1;
	spin_unlock_irqrestore(&psy->changed_lock, flags);
}
void power_supply_changed_zte(struct power_supply *psy)
{
	unsigned long flags;
	
	spin_lock_irqsave(&psy->changed_lock, flags);
	if (is_power_supply_changed) {
		psy->changed = true;
		wake_lock(&psy->work_wake_lock);
		is_power_supply_changed = 0;
		spin_unlock_irqrestore(&psy->changed_lock, flags);
		schedule_work(&psy->changed_work);
		printk("[real]psy change, psy name: %s\n",psy->name);
	} else
		spin_unlock_irqrestore(&psy->changed_lock, flags);
}
#if 0
...
#endif
EXPORT_SYMBOL_GPL(power_supply_changed_zte);
/**************************************************************************/
