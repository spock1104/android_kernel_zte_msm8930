			     ...
			     struct mmc_async_req *areq)
{
	...
	if (brq->data.error) {
		...
		//yeganlin_20140904,add trace
		trace_mmc_data_err(req->rq_disk->disk_name, brq->data.error,
		       (unsigned)blk_rq_pos(req),
		       (unsigned)blk_rq_sectors(req),
		       brq->cmd.resp[0], brq->stop.resp[0]);
...
//yeganlin
        MMC_FIXUP(CID_NAME_ANY, 0x45, CID_OEMID_ANY, add_quirk_mmc,
		  MMC_QUIRK_SANDISK_CHANGE_DATA_TIMEOUT),
	END_FIXUP
};
