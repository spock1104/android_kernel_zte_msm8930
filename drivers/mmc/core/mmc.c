...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 4) {
		...
		card->ext_csd.sec_feature_support =
			ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT];
//yeganlin_20131031,disable sanitize for sumsung emmc which will cause recovery failed
		if(card->ext_csd.rev == 6 && (card->cid.manfid == 0x15 ||card->cid.manfid == 0x45)){
			pr_err("mmc_read_ext_csd()::set sec_feature_support bit 6 to 0\n");
