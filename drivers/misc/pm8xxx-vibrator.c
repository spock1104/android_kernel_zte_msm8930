...
static int pm8xxx_vib_set(struct pm8xxx_vib *vib, int on)
{
	...
	if (on) {
		...
		//zte VIB 20140428 (/sys/module/pm8xxx_vibrator/parameters/vob_voltage)
		if(vibrator_voltage < (VIB_MAX_LEVEL_mV/100) &&  vibrator_voltage > (VIB_MIN_LEVEL_mV/100))
		{
			vib->level =vibrator_voltage;
			pr_info("vib_voltage rest to %d\n",vibrator_voltage);
		}
		pr_info("vib_on and voltage=%d\n", vib->level);
