			  ...
			  void *data)
{
	...
	ready_fw_ver=syna_get_fw_ver(syn_ts->client,syn_ts->fwfile[touch_moudle]);
#endif
		   ...
		   unsigned long count, void *data)
{
	...
	if(syna_fwupdate(syn_ts->client, syn_ts->fwfile[touch_moudle])<0)
	{
	...
	int *p_fw_ver )
{
	...
	pr_info("synaptics_get_configid chip_type=0x%x, sensor=0x%x, fw_ver=0x%x\n", 		
...
static int synaptics_rmi4_i2c_write(struct synaptics_rmi_data *rmi4_data,
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		if (i2c_transfer(rmi4_data->client->adapter, msg, 1) == 1) {
			retval = length;
			break;
		}
	...
	case TS_RESUME:	
		...
		//i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+3,0x10);
			...
			if(ts->config_id.sensor==0x31){//TPK
				...
				i2c_smbus_write_byte_data(ts->client,0xff,1);
				synaptics_rmi4_i2c_write(ts,0x11e,tpk_data,16);
				ret=i2c_smbus_read_byte_data(ts->client,0x72);
				i2c_smbus_write_byte_data(ts->client,0x72,ret|0x04);
...
static void synaptics_work_func(struct work_struct *work)
{
	...
	__u8 buf[buf_len];
#if defined (CONFIG_MACH_WARPLTE)|| defined (CONFIG_MACH_COEUS)	\
	|| defined (CONFIG_MACH_GAEA)||defined(CONFIG_MACH_NESTOR)\
	|| defined (CONFIG_MACH_PRMTHUS)
...
*/
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		for (i=0; i<ts->points_supported;i++)
		{
			...
			{   //up pointers , set z to 0 for resume log
				new_report_data[i].z = 0;
			}
		}
...
void zte_synaptics_change_fw_config( struct synaptics_rmi_data *ts )//xym
{

	#ifdef CONFIG_MACH_WARPLTE  //NOTICE just for N9510 not for P893V10
		i2c_smbus_write_byte_data(ts->client,0x5f,0xb0);	
		i2c_smbus_write_byte_data(ts->client,0xff,4);// change to page 3.
		i2c_smbus_write_byte_data(ts->client,0x01,0x0);
		//page 4 not need force update??
		i2c_smbus_write_byte_data(ts->client,0xff,0);// change to page 0.
	...
	const struct i2c_device_id *id)
{
	...
	synaptics_get_configid( ts, (char *)&chiptype,(char *)&sensor, &fw_ver);
	max_x = ts->max[0];
	max_y = ts->max[1];
	get_screeninfo(&xres, &yres);
	if ( ts->max_y_position != 0 )
	{
#if defined (CONFIG_MACH_COEUS) ||defined (CONFIG_MACH_WARPLTE) || defined (CONFIG_MACH_GAEA)\
	||defined(CONFIG_MACH_NESTOR)||defined(CONFIG_MACH_PRMTHUS)//P893A21 LCD 540*960 (laibao TS 707*1267)  (yushun TS 1150*2185)
	...
	zte_synaptics_change_fw_config(ts);//xym
