	...
	char *pfwfilename /*,u32 fwsize*/)
{
	...
	if (pbt_buf == NULL) {
		printk("syna_fw_update no memory\n");
		return -1;
	}	
...
int syna_fwupdate(struct i2c_client *client, char *pfwfilename ) 
{
	if ( !client || !pfwfilename )
		return -1;
	if(!strcmp(pfwfilename,""))
	{
		printk("syna_fwupdate pfwfilename is null\n");
		return -1;
	}
...
int syna_get_fw_ver(struct i2c_client *client, char *pfwfilename)
{
	...
	fwsize = syna_getfwsize(pfwfilename);
  	pbt_buf = (unsigned char *) kmalloc(fwsize+1,GFP_ATOMIC);
	if (pbt_buf == NULL) {
		printk("syna_get_fw_ver no memory\n");
		return -1;
	}
	...
	firmware_imgsize = synaptics_extract_long_from_header(&(pbt_buf[8]));
	config_imgdata = (unsigned char*) ( &pbt_buf[0] + 0x100 + firmware_imgsize );
