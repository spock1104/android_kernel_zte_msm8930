...
static u16 gain_trim_param = 120;
...
static u16 gain_trim_param = 220;
#elif defined(CONFIG_MACH_IRIS)
static u16 gain_trim_param = 170;
...
static ssize_t debug_strage(struct device *dev,
...
static ssize_t hi_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int hi = taos_cfgp->prox_threshold_hi;
	
	return sprintf(buf, "%d\n", hi);
}

static ssize_t hi_strage(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	unsigned long hi = 0;
	...
	if (strict_strtoul(buf, 10, &hi))
		return -EINVAL;
	printk("taos %s, write hi value:%lu",__func__,hi);
	if (!prox_on)			
	{
		printk(KERN_ERR "TAOS: fail, prox is off\n");
		return -EPERM;
	}
	if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG|TAOS_TRITON_CMD_SPL_FN|TAOS_TRITON_CMD_PROXALS_INTCLR)))) < 0) {
		printk(KERN_ERR "TAOS: i2c_smbus_write_byte failed\n");
	}
	if ((ret = (i2c_smbus_write_word_data(taos_datap->client, (0xA0 | TAOS_TRITON_PRX_MAXTHRESHLO),hi))) < 0) {    	       
		pr_crit(TAOS_TAG "set threshold_hi:%lu\n",hi);            			      	 
	}     
	if ((ret = (i2c_smbus_write_word_data(taos_datap->client, (0xA0 | TAOS_TRITON_PRX_MINTHRESHLO),taos_cfgp->prox_threshold_lo))) < 0) {    	       
		pr_crit(TAOS_TAG "set threshold_lo:%d\n",taos_cfgp->prox_threshold_lo);            			      	 
	}
	taos_cfgp->prox_threshold_hi = hi;
	return 1;
}

static ssize_t lo_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int lo = taos_cfgp->prox_threshold_lo;
	
	return sprintf(buf, "%d\n", lo);
}

static ssize_t lo_strage(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	unsigned long lo = 0;
	...
	if (strict_strtoul(buf, 10, &lo))
		return -EINVAL;
	printk("taos %s, write lo value:%lu",__func__,lo);
	if (!prox_on)			
	{
		printk(KERN_ERR "TAOS: fail, prox is off\n");
		return -EPERM;
	...
	if ((ret = (i2c_smbus_write_word_data(taos_datap->client, (0xA0 | TAOS_TRITON_PRX_MAXTHRESHLO),taos_cfgp->prox_threshold_hi))) < 0) {    	       
		pr_crit(TAOS_TAG "set threshold_hi:%d\n",taos_cfgp->prox_threshold_hi);            			      	 
	}     
	if ((ret = (i2c_smbus_write_word_data(taos_datap->client, (0xA0 | TAOS_TRITON_PRX_MINTHRESHLO),lo))) < 0) {    	       
		pr_crit(TAOS_TAG "set threshold_lo:%lu\n",lo);            			      	 
	}
	taos_cfgp->prox_threshold_lo = lo;
	return 1;
}

static ssize_t pulse_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int pulse_cnt = taos_cfgp->prox_pulse_cnt;
	
	return sprintf(buf, "%d\n", pulse_cnt);
}

static ssize_t pulse_strage(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	...
	if (strict_strtoul(buf, 10, &pulse_cnt))
		return -EINVAL;
	printk("taos %s, write pulse_cnt value:%lu",__func__,pulse_cnt);
	if (!prox_on)			
	{
		printk(KERN_ERR "TAOS: fail, prox is off\n");
		return -EPERM;
	}
	if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x0E), pulse_cnt))) < 0) {
             printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed\n");
                          
	}
	taos_cfgp->prox_pulse_cnt = pulse_cnt;
	prox_pulse_cnt_param = pulse_cnt;

	return 1;
}

static ssize_t gain_show_prox(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int gain = taos_cfgp->prox_gain;
	
	return sprintf(buf, "%d\n", gain);
}

static ssize_t gain_strage_prox(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	unsigned long gain;
	...
	if (strict_strtoul(buf, 10, &gain))
		return -EINVAL;
	printk("taos %s, write prox gain value:%lu",__func__,gain);
	...
	taos_cfgp->prox_gain = gain;
	prox_gain_param = gain;
	
	return 1;
}

//taos_cfgp->gain_trim = gain_trim_param;
static ssize_t gain_show_light(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int gain = taos_cfgp->gain_trim;
	
	return sprintf(buf, "%d\n", gain);
}

static ssize_t gain_strage_light(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t size)
{
	unsigned long gain;
	...
	if (strict_strtoul(buf, 10, &gain))
		return -EINVAL;
	printk("taos %s, write light gain_trim_param value:%lu",__func__,gain);


	taos_cfgp->gain_trim = gain;
	gain_trim_param = gain;
	
	return 1;
}

static struct device_attribute taos_attributes[] = {
  	__ATTR(debug, 0644, debug_show, debug_strage),
	__ATTR(hi, 0644, hi_show, hi_strage),
	__ATTR(lo, 0644, lo_show, lo_strage),
	__ATTR(pulse, 0644, pulse_show, pulse_strage),
	__ATTR(pgain, 0644, gain_show_prox, gain_strage_prox),
	__ATTR(lgain, 0644, gain_show_light, gain_strage_light),
};

static int create_sysfs_interfaces(struct device *dev)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(taos_attributes); i++)
		if (device_create_file(dev, taos_attributes + i))
			goto error;
	return 0;

error:
	for ( ; i >= 0; i--)
		device_remove_file(dev, taos_attributes + i);
	dev_err(dev, "%s:Unable to taos create interface\n", __func__);
	printk("%s, Unable to taos create interface\n",__func__);
	return -1;
}
...
*/
	...
	err = create_sysfs_interfaces(&clientp->dev);
	if (err < 0) {
		dev_err(&clientp->dev,
		   "device TAOS sysfs register failed\n");
...
///////////////end//////////////////////////////////////////////////
		...
		case TAOS_IOCTL_PROX_CALIBRATE:
			...
			//#if defined (CONFIG_MACH_METIS) || defined(CONFIG_MACH_NEX) || defined(CONFIG_MACH_STORMER)
