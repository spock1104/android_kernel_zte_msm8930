...
int pwm_enable(struct pwm_device *pwm)
{
	...
	if (!pwm->in_use) {
		...
		if (pwm_chip->is_lpg_supported) {
			...
			pm8xxx_pwm_bank_sel(pwm);
			rc = pm8xxx_pwm_bank_enable(pwm, 1);//zte copy form JB
			pm8xxx_pwm_start(pwm, 1, 0);
			if (pwm->chip->is_pwm_enable_sync_workaround_needed) // zte copy from JB
...
int pm8xxx_pwm_lut_enable(struct pwm_device *pwm, int start)
{
	...
	if (start) {
		...
		pm8xxx_pwm_bank_sel(pwm);//ZTE COPY FROM JB
...
static int __devinit pm8xxx_pwm_probe(struct platform_device *pdev)
{
	...
	if (version == PM8XXX_VERSION_8038) //zte
