...
/*
 * Added for sys interface of sensor info, i.e., id & name
 * by ZTE_JIA_20140529 jia.jia
 */
#if 1
int32_t msm_sensor_bayer_module_read(struct msm_sensor_ctrl_t *s_ctrl)
{
	s_ctrl = s_ctrl;
 ...
 */
#if 1
	if (s_ctrl->func_tbl->sensor_module_read)
		rc = s_ctrl->func_tbl->sensor_module_read(s_ctrl);
	else
		rc = msm_sensor_bayer_module_read(s_ctrl);
	if (rc < 0)
		goto probe_fail;
#endif
