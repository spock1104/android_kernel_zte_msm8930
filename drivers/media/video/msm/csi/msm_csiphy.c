...
/*
 * Merged from msm8930 jb for vfe32_irq to fix camif errors by caidezun 20140609
 *
 * Commented by ZTE_JIA_20140610 jia.jia
 */
#if defined(CONFIG_HI542_BAYER)
#define HI542_CAMIF_VFE32_IRQ_ERROR_PATCH
#endif
 ...
 * Commented by ZTE_JIA_20140610 jia.jia
 */
#if defined(HI542_CAMIF_VFE32_IRQ_ERROR_PATCH)
	uint32_t irq;
#endif
 ...
 */
#if defined(HI542_CAMIF_VFE32_IRQ_ERROR_PATCH)
	irq = msm_camera_io_r(csiphybase + MIPI_CSIPHY_LNCK_CFG4_ADDR);
	CDBG("%s MIPI_CSIPHY_LNCK_CFG4_ADDR = 0x%x\n", __func__, irq);

	msm_camera_io_w(0xc, csiphybase + MIPI_CSIPHY_LNCK_CFG4_ADDR);

	irq = msm_camera_io_r(csiphybase + MIPI_CSIPHY_LNCK_CFG4_ADDR);
	CDBG("%s 11 MIPI_CSIPHY_LNCK_CFG4_ADDR = 0x%x\n", __func__, irq);
#endif
