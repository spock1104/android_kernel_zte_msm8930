...
//not used from 21130725 by huangjinyu
//if remove notice ATM_FW_NAME
...
#define FTC_FW_NAME "Ver11_P893A21_FT5306_0x55_20130709_app.bin"
...
#define SYN_FW_NAME "ZTE_P893A21_Success_PR1183396-s2202_32333042.img"
#define ATM_FW_NAME ""
...
#define SYNAPTICS_MAX_Y_POSITION	2524
static ssize_t cap_ts_vkeys_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	return sprintf(
		buf,__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":120:1360:200:140"
...
#elif defined (CONFIG_MACH_GAEA)||defined(CONFIG_MACH_NESTOR)||defined(CONFIG_MACH_PRMTHUS)
#define SYNAPTICS_MAX_Y_POSITION	1962
static ssize_t cap_ts_vkeys_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	return sprintf(
...
*/
#endif
		ret = gpio_request(GPIO_TS_RST, "touch reset");
		if (ret){
			pr_err(" gpio %d request is error!\n", GPIO_TS_RST);
			return -1;
		}

		ret = gpio_request(GPIO_TS_IRQ, "touch irq");
		if (ret){
			pr_err("gpio %d request is error!\n", GPIO_TS_IRQ);
			return -1;
		}
