...
//zte PM (add DEBUG_SUSPEND 20140515)
static int debug_mask = DEBUG_USER_STATE | DEBUG_SUSPEND;
module_param_named(debug_mask, debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);

//zte PM(acount sleep/wake time 20140515)
...
#endif
//zte PM(acount sleep/wake time 20140515)-end
...
//zte PM 20140522 cp from JB20121024
			if (debug_mask & DEBUG_SUSPEND)
...
static void late_resume(struct work_struct *work)
{
	...
	//zte PM (cpu_freq 20140515)
	#ifdef CONFIG_ZTE_PLATFORM
	cpufreq_set_min_freq(1);//set to max freq when late_resume
    #endif
...
abort:
	...
	//zte PM (cpu_freq 20140515)
	#ifdef CONFIG_ZTE_PLATFORM
	cpufreq_set_min_freq(0);//set to min freq when out of late_resume
    #endif
}
...
void request_suspend_state(suspend_state_t new_state)
{
	unsigned long irqflags;
	int old_sleep;
	int wq_status = -1;//zte PM (for print log)
	...
	//zte PM (hall)
	#ifdef CONFIG_ZTE_HALL_AVAILABLE
	new_state_backup = new_state; /////liukejing
	#endif
